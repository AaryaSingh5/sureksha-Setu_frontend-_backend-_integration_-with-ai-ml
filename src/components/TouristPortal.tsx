...
export const TouristPortal: React.FC<TouristPortalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'itinerary' | 'route_finder'>('overview');
  ...
  const [routeDest, setRouteDest] = useState('Solang Riverbank & Avalanche Slope');
  const [selectedRouteIndex, setSelectedRouteIndex] = useState(0);

  // Place Safety Evaluation Helper
  const getPlaceSafetyEvaluation = (query: string) => {
    ...
    const matchedZone = geoFenceZones.find(z => 
      ...
      cleanQ.includes(z.name.toLowerCase().split(' ')[0]) ||
      (cleanQ.includes('rohtang') && (z.id === 'zone-rohtang' || z.name.toLowerCase().includes('rohtang'))) ||
      (cleanQ.includes('solang') && (z.id === 'zone-1' || z.name.toLowerCase().includes('solang'))) ||
      (cleanQ.includes('hadimba') && (z.id === 'zone-2' || z.name.toLowerCase().includes('hadimba'))) ||
      (cleanQ.includes('jogini') && (z.id === 'zone-jogini' || z.name.toLowerCase().includes('jogini'))) ||
      (cleanQ.includes('manali') && (z.id === 'zone-3' || z.name.toLowerCase().includes('manali')))
    );
    ...
    if (matchedZone) {
      return {
        ...
        title: `${matchedZone.name}`,
        description: matchedZone.description
      };
    }

    if (cleanQ.includes('atal tunnel') || cleanQ.includes('sissu') || cleanQ.includes('kasol')) {
      return {
        isGeofenced: false,
        riskLevel: cleanQ.includes('atal') ? 'Caution' : 'Safe',
        title: `${query} (Evaluated Transit Area)`,
        description: cleanQ.includes('atal')
          ? 'Black ice advisory active during morning hours. Slow mountain speeds recommended.'
          : 'Monitored tourist valley. Standard safety protocol and mobile connectivity available.'
      };
    }
    ...
    return {
      ...
      title: `${query} (Standard Tourist Landmark)`,
      description: 'Location within Himachal Pradesh tourism network. Directions and live mapping active.'
    };
  };
      ...
      {!authenticatedUser ? (
        ...
        <div className="space-y-5 text-left">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">
            ...
            {/* RIGHT COLUMN (Cols 6 - 12): ITINERARY PLANNER */}
            <div className="lg:col-span-7 space-y-5">

              <div className="bg-white border-2 border-slate-200 rounded-2xl p-4 shadow-sm space-y-4 text-left">

                {/* ITINERARY PLANNER HEADER & CONTENT */}
            ...
            {/* FULL-WIDTH DEDICATED ROW: GOOGLE MAPS DIRECTIONS & GEO-FENCE WITH SIDE-BY-SIDE LAYOUT */}
            <div className="lg:col-span-12 bg-white border-2 border-slate-200 rounded-2xl p-5 shadow-sm space-y-4 text-left">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-start">

                {/* LEFT SIDE (Cols 1 - 5): GEOFENCE CONTROLS, DEBUG CONSOLE & PRESETS */}
                <div className="lg:col-span-5 space-y-3.5">
                  ...
                  {/* Geo-Fence Safety Zone Header & Status */}
                  ...
                  <div className="bg-slate-50 border border-slate-200 rounded-xl p-3 text-[10px] font-mono space-y-1.5">
                  ...
                  {/* Preset Zone Buttons (2 Rows: 3 on top, 2 below matching diagram) */}
                  <div className="space-y-1.5">
                    <div className="grid grid-cols-3 gap-1.5">
                      {geoFenceZones.slice(0, 3).map((zone) => (
                        <button
                          key={zone.id}
                          onClick={() => {
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      {geoFenceZones.slice(3, 5).map((zone) => (
                        <button
                          key={zone.id}
                          onClick={() => {
                            setActiveGeoFenceZone(zone);
                  ...
                  {/* Active GeoFence Warning Display Banner */}
                  {activeGeoFenceZone.riskLevel === 'Unsafe' ? (
                    <div className="p-3 rounded-xl bg-red-50 border border-red-200 text-red-900 text-[11px] font-medium flex items-start gap-2">
                      ...
                      <div>
                        ...
                        <div className="mt-0.5">{activeGeoFenceZone.description}</div>
                      </div>
                    </div>
                  ) : activeGeoFenceZone.riskLevel === 'Caution' ? (
                    ...
                    <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-200 text-emerald-900 text-[11px] font-medium flex items-start gap-2">
                ...
                {/* RIGHT SIDE (Cols 6 - 12): PLACE SAFETY SEARCH, DIRECTIONS & LARGE MAP */}
                <div className="lg:col-span-7 space-y-3">

                  {/* 1. SEARCH MAP PLACE / LANDMARK & SAFETY ANALYZER */}
                  <div className="space-y-1.5">
                    <label className="block text-[10px] font-extrabold text-slate-600 uppercase">
                      Search Map Place / Landmark (Safety Evaluation):
                    </label>
                    <div className="flex gap-1.5">
                      <div className="relative flex-1">
                        <input
                          ...
                          placeholder="Search e.g. Hadimba Temple, Rohtang, Kasol, Solang..."
                    ...
                    {/* Real-Time Place Safety Evaluation Result Card */}
                    {(() => {
                      const targetEval = getPlaceSafetyEvaluation(mapSearchQuery || routeDest);
                      if (!targetEval) return null;

                      return (
                        <div
                          className={`p-2.5 rounded-xl border text-xs font-medium flex items-start justify-between gap-2 shadow-2xs transition-all ${
                            targetEval.riskLevel === 'Unsafe'
                              ? 'bg-red-50/90 border-red-300 text-red-950'
                              : targetEval.riskLevel === 'Caution'
                        ...
                        >
                          <div className="flex items-start gap-2">
                            {targetEval.riskLevel === 'Unsafe' ? (
                              <ShieldAlert className="w-4 h-4 text-red-600 flex-shrink-0 mt-0.5 animate-bounce" />
                            ) : targetEval.riskLevel === 'Caution' ? (
                              <AlertTriangle className="w-4 h-4 text-amber-600 flex-shrink-0 mt-0.5" />
                            ) : (
                              <ShieldCheck className="w-4 h-4 text-emerald-600 flex-shrink-0 mt-0.5" />
                            ...
                            <div>
                              <div className="font-extrabold flex items-center gap-1.5 text-[11px]">
                                ...
                                >
                                  {targetEval.riskLevel === 'Unsafe' ? 'HIGH RISK' : targetEval.riskLevel.toUpperCase()}
                                </span>
                              </div>
                              <p className="text-[10.5px] opacity-90 mt-0.5">{targetEval.description}</p>
                            </div>
                          </div>

                          <button
                            onClick={() => {
                              setRouteDest(mapSearchQuery || targetEval.title);
                            }}
                            className="px-2 py-1 rounded-lg bg-white/80 hover:bg-white text-[10px] font-extrabold border border-slate-300 shadow-2xs flex-shrink-0"
                          >
                            Set Destination
                          </button>
                        </div>
                  ...
                  {/* 2. Route Directions: Origin ➔ Destination */}
                  <div className="space-y-1">
                    <label className="block text-[10px] font-extrabold text-slate-600 uppercase">
                  ...
                  {/* 3. Actual Google Map Component with Large Height & Geofence Overlay */}
                  <ActualGoogleMap
                  ...
                  <div className="flex items-center justify-between text-[11px] text-slate-600 bg-slate-50 p-2.5 rounded-xl border border-slate-200 font-semibold">
                    <span>Active Monitored Sector: <strong className="text-slate-900">{activeGeoFenceZone.name}</strong></span>
